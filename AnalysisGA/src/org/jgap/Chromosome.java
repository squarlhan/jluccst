 ...
 * implementation of the Gene interface, but all genes in a respective position
 * (locus) must share the same concrete implementation across Chromosomes within
 * a single population (genotype). In other words, gene 1 in a chromosome must
 * share the same concrete implementation as gene 1 in all other chromosomes in
 * the population.
 * 
 * @author Neil Rotstan
...
public class Chromosome extends BaseChromosome {
	...
	/**
	 * Application-specific data that is attached to this Chromosome. This data
	 * may assist the application in evaluating this Chromosome in the fitness
	 * function. JGAP does not operate on the data, aside from allowing it to be
	 * set and retrieved, and considering it with comparations (if user opted in
	 * to do so).
	 */
	 ...
	 * fitness function. A value of -1 indicates that this field has not yet
	 * been set with this Chromosome's fitness values (valid fitness values are
	 * always positive).
	 * 
	...
	/**
	 * Optional helper class for checking if a given allele value to be set for
	 * a given gene is valid. If not, the allele value may not be set for the
	...
	/**
	 * If set to true the method getFitnessValue() will always (re-)calculate
	 * the fitness value. This may be necessary in case of environments where
	 * the state changes without the chromosome to notice
	 * 
	 * @since 3.2.2
	 */
	 ...
	 * @param a_configuration
	 *            the configuration to use
	 * @param a_persistentRepresentatuion
	 *            valid persistent representation that was most likely obtained
	 *            via getPersistentRepresentation()
	 * @throws InvalidConfigurationException
	 ...
	 *            the configuration to use
	 * @param a_desiredSize
	 *            number of genes the chromosome contains of
	 * @throws InvalidConfigurationException
	 * 
	 * @author Klaus Meffert
	 * @since 2.2
	 */
	public Chromosome(final Configuration a_configuration,
			final int a_desiredSize) throws InvalidConfigurationException {
		this(a_configuration);
		if (a_desiredSize <= 0) {
	 ...
	 * construct a new Chromosome instance containing genes all of the same type
	 * as the sample Gene. This can be useful for constructing sample
	 ...
	 *            the configuration to use
	 * @param a_sampleGene
	 *            a concrete sampleGene instance that will be used as a template
	 *            for all of the genes in this Chromosome
	 * @param a_desiredSize
	 *            the desired size (number of genes) of this Chromosome
	 * @throws InvalidConfigurationException
	 * 
	 * @author Neil Rotstan
	 * @author Klaus Meffert
	...
	public Chromosome(final Configuration a_configuration, Gene a_sampleGene,
			int a_desiredSize, IGeneConstraintChecker a_constraintChecker)
			throws InvalidConfigurationException {
		this(a_configuration, a_desiredSize);
		initFromGene(a_sampleGene);
	 ...
	 * can be useful for constructing sample chromosomes that are to be used to
	 * setup a Configuration object.
	 * 
	 * @param a_configuration
	 *            the configuration to use
	 * @param a_initialGenes
	 *            the initial genes of this Chromosome
	 * @throws InvalidConfigurationException
	 * 
	 * @author Neil Rotstan
	 * @since 1.0
	...
	public Chromosome(final Configuration a_configuration, Gene[] a_initialGenes)
			throws InvalidConfigurationException {
		this(a_configuration, a_initialGenes == null ? 0
				: a_initialGenes.length);
	 ...
	 * Constructs a Chromosome separate from any specific Configuration. This
	 * can be useful for constructing sample chromosomes that are to be used to
	 * setup a Configuration object. Additionally, a constraint checker can be
	 ...
	 * @param a_initialGenes
	 *            the initial genes of this Chromosome
	 * @param a_constraintChecker
	 *            constraint checker to use
	 * @throws InvalidConfigurationException
	 *             in case the constraint checker reports a configuration error
	 * 
	 * @author Klaus Meffert
	 * @since 2.5
	 */
	public Chromosome(final Configuration a_configuration,
			Gene[] a_initialGenes, IGeneConstraintChecker a_constraintChecker)
			throws InvalidConfigurationException {
	 ...
	 * 
	 * @param a_initialGenes
	 *            the initial genes of this Chromosome to verify
	 * 
	 * @author Klaus Meffert
	 * @since 2.5
	 */
	protected void checkGenes(Gene[] a_initialGenes) {
		...
		for (int i = 0; i < a_initialGenes.length; i++) {
			if (a_initialGenes[i] == null) {
				throw new IllegalArgumentException("The gene at index " + i
	 ...
	 * independently of this instance. Note that, if possible, this method will
	 * first attempt to acquire a Chromosome instance from the active
	 * ChromosomePool (if any) and set its value appropriately before returning
	 * it. If that is not possible, then a new Chromosome instance will be
	 * constructed and its value set appropriately before returning.
	 * 
	 * @return copy of this Chromosome
	 * @throws IllegalStateException
	 *             instead of CloneNotSupportedException
	 * 
	 * @author Neil Rotstan
	 * @author Klaus Meffert
	 * @since 1.0
	...
	public synchronized Object clone() {
		...
		try {
			if (copy == null) {
				...
				// create
				// a new one. First we make a copy of each of the Genes. We
				// explicity
				// use the Gene at each respective gene location (locus) to
				// create the
				// new Gene that is to occupy that same locus in the new
				// Chromosome.
				// -------------------------------------------------------------------
				int size = size();
				if (size > 0) {
					...
					for (int i = 0; i < size; i++) {
						...
						if (allele != null) {
							IJGAPFactory factory = getConfiguration()
									.getJGAPFactory();
							if (factory != null) {
								ICloneHandler cloner = factory
										.getCloneHandlerFor(allele,
												allele.getClass());
								if (cloner != null) {
									try {
										allele = cloner.perform(allele, null,
												this);
									} catch (Exception ex) {
					...
					// Now construct a new Chromosome with the copies of the
					// genes and
					// return it. Also clone the IApplicationData object later
					// on.
					// ---------------------------------------------------------------
					if (getClass() == Chromosome.class) {
						...
						try {
							// Try dynamic call of constructor. Attention: This
							// may not
							// work for inner classes!
							// --------------------------------------------------------
							Constructor[] constr = getClass()
									.getDeclaredConstructors();
							if (constr != null) {
								for (int i = 0; i < constr.length; i++) {
									Class[] params = constr[i]
											.getParameterTypes();
									if (params != null && params.length == 1) {
										if (params[0] == Configuration.class) {
											copy = (IChromosome) constr[i]
													.newInstance(new Object[] { getConfiguration() });
							...
							if (copy == null) {
								// Enforce alternative cloning to get at least
								// something.
								// ------------------------------------------------------
								throw new Exception(
							...
							ex.printStackTrace();
							// Do it the old way with the danger of getting a
							// stack overflow.
							// --------------------------------------------------------------
							copy = (IChromosome) getConfiguration()
									.getSampleChromosome().clone();
							copy.setGenes(copyOfGenes);
						}
					}
				} else {
		...
		if (m_multiObjective != null) {
			if (getClass() == Chromosome.class) {
				try {
					((Chromosome) copy)
							.setMultiObjectives((List) cloneObject(m_multiObjective));
				} catch (Exception ex) {
					throw new IllegalStateException(ex.getMessage());
				}
			}
		}
		((Chromosome) copy).setPap(pap);
		((Chromosome) copy).setMom(mom);
		((Chromosome) copy).setIndex(index);
		return copy;
	}

	/**
	 * Clones an object by using clone handlers. If no deep cloning possible,
	...
	/**
	 * Static convenience method. Clones an object by using clone handlers. If
	 * no deep cloning possible, then return the reference.
	 * 
	 * @param a_config
	 *            a valid configuration to obtain the JGAPFactory from
	 * @param a_object
	 *            the object to clone
	 * @param a_master
	 *            the super object of a_object, e.g. a chromosome in case of
	...
	public static Object cloneObject(Configuration a_config, Object a_object,
			Object a_master) throws Exception {
		if (a_object == null) {
			return null;
		}
		// Try to clone via a registered clone handler.
		// --------------------------------------------
		ICloneHandler cloner = a_config.getJGAPFactory().getCloneHandlerFor(
				a_object, a_object.getClass());
		if (cloner != null) {
	 ...
	 * active fitness function. If a bulk fitness function is in use and has not
	 * yet assigned a fitness value to this Chromosome, then -1 is returned.
	 * <p>
	 * Attention: should not be called from toString() as the fitness value
	 * would be computed if it was initial!
	 * 
	...
	/**
	 * @return the lastly computed fitness value, or
	 *         FitnessFunction.NO_FITNESS_VALUE in case no value has been
	 *         computed yet.
	 * 
	 * @author Klaus Meffert
	 */
	...
	protected double calcFitnessValue() {
		if (getConfiguration() != null) {
			...
			if (normalFitnessFunction != null) {
				// Grab the "normal" fitness function and ask it to calculate
				// our
				// fitness value.
				// --------------------------------------------------------------
	 ...
	 * fitness functions and should not be invokved from anything else (except
	 * test cases).
	 * 
	 * @param a_newFitnessValue
	 *            a positive integer representing the fitness of this Chromosome
	 * 
	 * @author Neil Rotstan
	 * @since 1.0
	 */
	...
	/**
	 * Sets the fitness value of this Chromosome directly without any constraint
	 * checks, conversions or checks. Only use if you know what you do.
	 * 
	 * @param a_newFitnessValue
	 *            a positive integer representing the fitness of this Chromosome
	 * 
	 ...
	 *             if the given Configuration instance is null
	 * 
	 * @author Neil Rotstan
	 * @author Klaus Meffert
	 * @since 1.0
	...
	public boolean equals(Object other) {
		...
		// -----------------------------------------------------------------
		/*
		 * if (other != null && !this.getClass ().getName ().equals
		 * (other.getClass ().getName ())) { return false; }
		 */
		try {
	...
	/**
	 * Retrieve a hash code for this Chromosome. Does not considers the order of
	 * the Genes for all cases (especially when gene is empty).
	 * 
	...
	public int compareTo(Object other) {
		...
		if (otherChromosome.getPap() != pap) {
			return 1;
		}
		if (otherChromosome.getMom() != mom) {
			return 1;
		}

		if (otherChromosome.getIndex() != index) {
			return 1;
		}
	 ...
	 * continue to the next generation or manually (e.g. via an add-method).
	 * 
	 * @param a_isSelected
	...
	/**
	 * Invoked when this Chromosome is no longer needed and should perform any
	 * necessary cleanup. Note that this method will attempt to release this
	 * Chromosome instance to the active ChromosomePool, if any.
	 * 
	 * @author Neil Rotstan
	 * @since 1.0
	 */
	 ...
	 * Chromosome. Attaching application-specific data may be useful for some
	 * applications when it comes time to evaluate this Chromosome in the
	 * fitness function. JGAP ignores this data, except for cloning and
	 ...
	 * Chromosome. Attaching application-specific data may be useful for some
	 * applications when it comes time to evaluate this Chromosome in the
	 * fitness function. JGAP ignores this data functionally.
	 * 
	 ...
	 * 
	 * @param a_genes
	 *            the genes to set for the chromosome
	 * 
	 * @throws InvalidConfigurationException
	 *             in case constraint checker is provided
	 * 
	 * @author Klaus Meffert
	 */
	 ...
	 * 
	 * @param a_constraintChecker
	 *            the constraint checker to be set
	 * @throws InvalidConfigurationException
	 * 
	 * @author Klaus Meffert
	 * @since 2.5
	...
	/**
	 * Verifies the state of the chromosome. Especially takes care of the given
	 * constraint checker.
	 * 
	 * @param a_constraintChecker
	 *            the constraint checker to verify
	 * 
	 * @throws InvalidConfigurationException
	 * 
	 * @author Klaus Meffert
