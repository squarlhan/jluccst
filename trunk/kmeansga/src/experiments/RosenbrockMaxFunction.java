...
public class RosenbrockMaxFunction 
  ...
  public double evaluate(IChromosome a_subject) {
    ...
    for (int i = 0; i < n-1; i++) {
    	total+=(100*Math.pow((((DoubleGene)a_subject.getGene(i+1)).doubleValue()
    			-Math.pow(((DoubleGene)a_subject.getGene(i)).doubleValue(), 2)), 2)
    			+Math.pow((((DoubleGene)a_subject.getGene(i)).doubleValue()-1), 2));
