...
<div class="dtree">
  ...
  <script type="text/javascript">
		...
		<%}else{%>
			d.add(0,-1,'<s:property value="companyName"/>','./openSellReportStatTargetAction.action?companyId=<s:property value="companyId"/>&statYear=<%=Calendar.getInstance().get(Calendar.YEAR)%>','','groupmain');
		<%}%>
		<s:iterator value="deptList">
			d.add("<s:property value="id"/>",0,'<s:property value="deptName"/>','./openSellReportStatTargetAction.action?companyId=<s:property value="companyId"/>&deptId=<s:property value="id"/>&statYear=<%=Calendar.getInstance().get(Calendar.YEAR)%>','','groupmain','<%=path%>/js/tree/img/group.gif','<%=path%>/js/tree/img/group.gif');
		</s:iterator>
		<s:iterator value="userList">
			d.add("<s:property value="id"/>","<s:property value="deptId"/>",'<s:property value="userCName"/>','./openSellReportStatTargetAction.action?companyId=<s:property value="companyId"/>&deptId=<s:property value="deptId"/>&personId=<s:property value="id"/>&statYear=<%=Calendar.getInstance().get(Calendar.YEAR)%>','','groupmain','<%=path%>/js/tree/img/user.gif');
		</s:iterator>
		document.write(d);
		//-->
	</script>
